...
def main():
    victims = setUpGame()
    players = get_live_players(victims)
    #print(players)
    gameEngine(players)
...
def setUpGame(): # defines player_info function
    ...
    num_of_players = input("How many players are there? ") # this request user input of an int and set the value equal to the variable numPlayers

    for x in range(num_of_players): # this for loop iterates for the number of the range of numPlayers
        user_name = raw_input("Enter username for Player %s: " % (x + 1)) # this requests user input of a string(want username) and sets it equal to user
...
# use a empty list for live_players and find what players are still alive by iterating through the player dictionary and checking the life attribute and then return the players that are alive

#create while loop that while set to the alive players list(so while there is still a player still alive) run the gameEngine
def get_live_players(players):
    ...
    for a in players:
        ...
        if players[a].life == True:
            #print('hello')
            live_players.append(players[a].name)
    return live_players

#while live_players: # while loop. while player attribute life is true(alive)
def gameEngine(players):
    ...
    for i in players: # for loop that iterates through each player in game
        ...
        if current_chapter == '2b' or current_chapter == '7b' or current_chapter == '10a' or current_chapter == '10b':  # if statement that determines if the current chapter is equal to a dead end chapter(where the player will die)
            ...
            print storyline.chapters[current_chapter] # prints current_chapter by using chapters() from text file
            players[i].set_choice(raw_input("Player: " + current_player + "Choose a or b: ")) # requests string input from user and sets it equal to their choice of that player
...
#YOU ARE HERE! use if  statements to jump chapters
...
# ask for players' names   # setUpGame()
#
# print chapter to a player
# ask for their choice
# *repeat for each player
# *repeat until player dies or game ends
# print GAME OVER   # gameEngine()
